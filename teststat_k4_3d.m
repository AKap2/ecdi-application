function z=teststat_k4_3d(X0_ord_rep,X1_rep,pol,o ,u,p,test)
%TESTSTAT_K4_3D returns z=teststatistic of location polynomial at locations 0:1/(p-1):1
% and the deviations from quantile bounds for three dimensional data with location polynomials of degree four. Interpolation between copula samples X0 and X1
% Inputs:
...
for i=1:(p-2) %do not test margins of X0 and X1
   z=[z teststat(X2(m/3*i+1:m/3*i+m/3,1),m/3,test)-o-pol2(1+6*(i-1))];
   z=[z u-teststat(X2(m/3*i+1:m/3*i+m/3,1),m/3,test)-pol2(2+6*(i-1));];
z=[z teststat(X2(m/3*i+1:m/3*i+m/3,2),m/3,test)-o-pol2(3+6*(i-1))];
z=[z u-teststat(X2(m/3*i+1:m/3*i+m/3,2),m/3,test)-pol2(4+6*(i-1))];
z=[z teststat(X2(m/3*i+1:m/3*i+m/3,3),m/3,test)-o-pol2(5+6*(i-1))];
z=[z u-teststat(X2(m/3*i+1:m/3*i+m/3,3),m/3,test)-pol2(6+6*(i-1))];
end
z=[z -pol2'];