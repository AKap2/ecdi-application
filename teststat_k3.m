function z=teststat_k3(X0_ord_rep,X1_rep,pol,o ,u,p,test)
%TESTSTAT_K3 returns z=teststatistic of location polynomial at locations 0:1/(p-1):1
% and the deviations from quantile bounds for two dimensional data with location polynomials of degree three. Interpolation between copula samples X0 and X1
% Inputs:
...
z=[];
for i=1:(p-2) 
   z=[z teststat(X2(m/2*i+1:m/2*i+m/2,1),m/2,test)-o-pol2(1+4*(i-1))];
   z=[z u-teststat(X2(m/2*i+1:m/2*i+m/2,1),m/2,test)-pol2(2+4*(i-1));];
z=[z teststat(X2(m/2*i+1:m/2*i+m/2,2),m/2,test)-o-pol2(3+4*(i-1))];
z=[z u-teststat(X2(m/2*i+1:m/2*i+m/2,2),m/2,test)-pol2(4+4*(i-1))];
end
z=[z -pol2'];
